---
title: 'Cilium'
originalFilePath: 'src/cncf-projects/cilium.md'
---
...
In the following examples, we demonstrate how Cilium can be used to secure a EDB Postgres for Kubernetes PostgreSQL instance by
...
metadata:
  name: postgresql-operator-operator-policy
  namespace: default
spec:
...
!!! Important
    The `postgresql-operator-system` namespace is the default namespace for the operator when using the YAML manifests, if the operator
...
kind: CiliumNetworkPolicy
metadata:
  name: postgresql-operator-policy
  namespace: default
spec:
...
The policy allows access from `postgresql-operator-system` Pods and from `default` namespace Pods that also belong to `cluster-example`.
...
PostgreSQL instance managed by EDB Postgres for Kubernetes via port 5432 in the `default` namespace.
